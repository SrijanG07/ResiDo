...
function WishlistPage({ onViewProperty, onNavigate }) {
    ...
    if (loading) {
        return <div className="wishlist-page loading">Loading your wishlist...</div>;
    }
                ...
                <>
                    {compareMode && (
                        <div className="compare-bar">
                            <span>Select up to 3 properties to compare ({selectedForCompare.length}/3)</span>
                        ...
                        {wishlist.map(property => (
                            ...
                            >
                                ...
                                <div className="card-image">
                                    <img 
                                        src={property.images?.[0]?.image_url || property.images?.[0]?.url || 'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=400&h=300&fit=crop'} 
                                        alt={property.title}
                                    />
                                    <button 
                                ...
                                <div className="card-content">
                                    ...
                                    {!compareMode && (
                                        <button className="btn btn-outline btn-sm" onClick={(e) => { e.stopPropagation(); onViewProperty && onViewProperty(property.id); }}>
                                            View Details
                                        </button>
                                    )}
                                </div>
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        <tr>
                                            <td>Price</td>
                                            {getCompareItems().map(p => (
                                                <td key={p.id} className="highlight">{formatPrice(p.price)}</td>
                                            ))}
                                        </tr>
                                        <tr>
                                            <td>Size</td>
                                        ...
                                        <tr>
                                            <td>Location</td>
                                            {getCompareItems().map(p => (
                                                <td key={p.id}>{p.locality}, {p.city}</td>
                                            ))}
                                        </tr>
                                        <tr>
                                            <td>Type</td>
                                            {getCompareItems().map(p => (
                                                <td key={p.id} className="capitalize">{p.property_type}</td>
                                            ))}
                                        </tr>
                                        <tr>
                                            <td>Furnished</td>
                                            {getCompareItems().map(p => (
                                                <td key={p.id} className="capitalize">{p.furnished?.replace('-', ' ')}</td>
                                            ))}
                                        </tr>
                                        <tr>
                                            <td>Price/sq.ft</td>
                                            {getCompareItems().map(p => (
                                                <td key={p.id}>₹{Math.round(p.price / (p.size || 1)).toLocaleString()}</td>
                                            ))}
                                        </tr>
                                    </tbody>
                                </table>
